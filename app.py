...
from bottle import route, template, run, static_file, request, response, redirect
@route('/')
def handle_root_url():
...
def handle_form():
    if 'verified email' in request.cookies:
        ask_email = False
    else:
        ask_email = True
    status = ''
    ...
    if request.POST:
        ...
        user = email + '/token'
        api_token = 'add_token'
        url = 'https://add_url.zendesk.com/api/v2/requests.json'
